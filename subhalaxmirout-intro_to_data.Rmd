...
```{r}
library(ggplot2)
ggplot(cdc) + geom_point(aes(x = cdc$weight, y = cdc$wtdesire))
```
 The scatter plot shows a positive linear relationship between weight and desired weight, when weight increases desired weight also increases.
...
>  Let new variable name wdiff.
   wdiff = desired weight - weight
```{r}
...
typeof(wdiff)
```
> The type of 'wdiff' variable is **numeric**. 
If 'wdiff' is 0, it means, the person’s desired weight and his current weight are same. If 'wdiff' > 0 (positive), it means the person wants to put on more weight than his current weight and similarly if wtdiff < 0 (negative), it means the person wants to lose weight than his current weight.
...
```{r eval=TRUE}
summary(wdiff)
boxplot(wdiff, main = "Difference between desired weight and weight")
hist(wdiff,main = "Difference between desired weight and weight", xlab = "Difference weight", col = "sky blue",breaks = 100 )
```
 From the hitogram, we can see, this is a unimodal distribution.Persons want to reduce more weight than their actual weight. 

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
```{r}
boxplot(wdiff ~ cdc$gender, main = "Differences between desired weight and actual  weight by gender")
male <- subset(cdc, cdc$gender == 'm')
head(male)
female <- subset(cdc, cdc$gender == 'f')
head(female)
male_wdiff <- male$wtdesire - male$weight
female_wdiff <- female$wtdesire - female$weight
summary(male_wdiff)
summary(female_wdiff)
```
From the summary and box plot, we can see, compair to men women wants to loose more weight. 
On average,
Women weight difference mean : 18.15. 
Men weight difference mean: 10.71.
...
sd <- sd(cdc$weight)
weight_lower <- mean - sd
weight_upper <- mean + sd
std_wt <- subset(cdc, (weight > weight_lower) & (weight < weight_upper))
proper_std_wt <- dim(std_wt)[1]/dim(cdc)[1]
proper_std_wt
hist(cdc$weight, main = "Weight")
```
 From the above shows that 70%  of the weights are within one standard deviation of the mean.
